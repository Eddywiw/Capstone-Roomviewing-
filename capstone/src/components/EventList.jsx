      ...
      <div className="list-div">
        ...
        {noScheduleMessage ? (
          ...
          <ul>
            {events.map((event) => (
              <li key={event.id}>
                Room no: {event.Roomno} | {event.Title} - {event.Professor} <br /> Date: {event.Start.toDate().toLocaleDateString()} | Start: {event.Start.toDate().toLocaleTimeString()} - End: {event.End.toDate().toLocaleTimeString()}<br/>Section: {event.Section} 
              </li>
            ))}
          </ul>
        )}
